...
void create_file(const char *filename, int *n, double *x, FILE *f){
    ...
    do{
        printf("Введите колличество чисел от 1 до 500:");
        f_err = scanf("%d", n);
        f_err2 = 0;
        char chr = getchar();
        while(chr!='\n') {
        ...
        if (n <= 0 || *n > 500 || f_err != 1 || f_err2 == 1) {
            printf("\nОшибка: некорректное колличество чисел, попробуйте снова.\n");
        }
    } while (n <= 0 || *n > 500 || f_err != 1 || f_err2 == 1);
...
void print_result(double result){
    if (result == 0.0){
        ...
        printf("Среднее арифметическое положительных: %.2lf\n", result);
    }
}
